...
df = pd.read_excel(r'C:\Users\Sanuj Goyal\OneDrive\Desktop\Population Prediction Project\Japan_output.xlsx')
print(df)

##################################Converting string data to numeric data#############################################

df['Averagepopulation'] = df['Averagepopulation'].str.replace(',', '').astype(int)
df['Live births'] = df['Live births'].str.replace(',', '').astype(int)
df['Deaths'] = df['Deaths'].str.replace(',', '').astype(int)
df['Naturalchange'] = df['Naturalchange'].str.replace(',', '').astype(int)
print(df)

##################################Converting data colums to lists####################################################

year = df['Year'].tolist()
population = df['Averagepopulation'].tolist()
births = df['Live births'].tolist()
deaths = df['Deaths'].tolist()
natural_change = df['Naturalchange'].tolist()
