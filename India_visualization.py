...
df = pd.read_excel(
    r'C:\Users\Sanuj Goyal\OneDrive\Desktop\Population Prediction Project\India_output.xlsx')
print(df)
...
df['Mid-year population'] = df['Mid-year population'].str.replace(
    ',', '').astype(int)
...
year = df['Year'].tolist()
population = df['Mid-year population'].tolist()
...
plt.title('Population vs Year 1991-2020')
plt.xlabel('Year')
...
# Line Graph for Births, Deaths, Natural Change
plt.plot(x_axis, births, label='Annual Births')
plt.plot(x_axis, deaths, label='Annual Deaths')
plt.plot(x_axis, natural_change, label='Natural Change')
plt.title('Births vs Death vs Year')
plt.xlabel('Year')
plt.ylabel('Births, Deaths')
plt.legend()
plt.show()
